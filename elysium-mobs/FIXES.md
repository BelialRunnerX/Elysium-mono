...
## 15. Thirty creatures from eight entity types

Six families — Scavenger, Reaver, Whisper on the Unsworn side; Drone, Lictor,
Adept on the Empire's — plus two bosses. The sub-variant is synced entity data
rather than a registration, so thirty creatures need six models and six
renderers instead of thirty of each. Vanilla does the same for cats and
villagers.

A variant is a texture, three multipliers and **one ability**. The ability is
the part that makes it a different fight; without it thirty variants would be
one creature with a slider.

## 16. The balance budget, and why it is checked

Every variant's health, damage and speed multipliers sum to exactly 3.00.

Five sub-variants stay a *choice* only if none is simply better, and the way
that promise breaks is a variant getting nudged up during tuning while nobody
adds the row back up. So `validate.py` adds up all thirty. Deliberately breaking
one to 3.50 produced the right complaint.

Abilities are not in the budget and cannot be — there is no exchange rate
between "+0.2 health" and "poisons on hit". The numbers carry the part that can
be checked; the abilities carry the part that cannot, and saying so is more
honest than inventing a weighting.

## 17. Two content mods that fill each other in without meeting

Elysium Dungeons needed creatures; Elysium Mobs has them. Both obvious wirings
are wrong: dungeons naming the mobs welds them together and stops dungeons
working alone, and mobs registering dungeon rooms means only that one mod can
ever populate a dungeon.

So the library gained `ElysiumBestiary` — creatures by faction and role. Mobs
registers into it, dungeons asks it, and neither imports the other. Install
either alone and both still work; a dungeon with no mob mod gets a plain
fallback boss rather than an empty boss room.

That is enforced rather than trusted. `validate.py` fails if anything under
`com.elysium.mobs` refers to another content mod, and `compile.sh` builds mobs
with dungeons deliberately off the classpath — so a direct reference cannot
compile even by accident. Adding an import of `com.elysium.dungeons` was
negative-tested and correctly rejected.

## 18. A generator that made the mistake it exists to prevent

Entity models and their textures are two halves of one fact: the model says a
box's faces are at (16, 8), and the sheet has to have them there. So both are
generated from one box table.

The first draft of that table had hand-assigned UVs, and the Reaver's legs
overlapped its body — two boxes whose *unwrapped footprints* collided, which no
amount of care catches by eye. UVs are now assigned by a shelf packer, and a
separate check walks every family's sheet pixel by pixel confirming no two
distinct boxes claim the same pixel. Zero overlaps across all six.

Also fixed there: the accent band crashed on boxes narrower than four pixels
(the Drone's fins), because the rectangle's x1 fell below its x0.

## 19. What the harness cannot reach

Rendering. Everything else in five projects is compiled and validated; the
models and textures are correct by construction and have never been drawn on a
screen. That limit is worth stating plainly rather than letting a green
validator imply more than it checked.
